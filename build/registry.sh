...
if [ "$provider" = "aws" ]; then
  registry_push_url="public.ecr.aws/x6x7z2q3"
fi
...
function registry_login() {
  if [ "$is_registry_logged_in" = "false" ]; then
    if [ "$provider" = "aws" ]; then
      blue_echo "public.ecr.aws/x6x7z2q3/mor-cortex..."
...
function create_aws_registry() {
  for image in "${aws_images[@]}"; do
    aws ecr-public create-repository --repository-name=mor-cortex/$image --region=us-east-1 || true
  done
}
...
function build() {
  ...
  if [ -n "$AWS_ACCOUNT_ID" ] && [ -n "$AWS_REGION" ]; then
    tag_args+=" -t public.ecr.aws/x6x7z2q3/mor-cortex/$image:$tag"
  fi

  blue_echo "Building $image:$tag..."
  docker build $ROOT -f $dir/Dockerfile -t mor-cortex/$image:$tag $tag_args $build_args
...
function cache_builder() {
  ...
  docker build $ROOT -f $dir/Dockerfile -t mor-cortex/$image-builder:$CORTEX_VERSION --target builder
...
function push() {
  ...
  docker push $registry_push_url/mor-cortex/$image:$tag
